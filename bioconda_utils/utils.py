...
def ellipsize_recipes(
    ...
    return (
        " ("
        + ", ".join(recipe.lstrip(recipe_folder).lstrip("/") for recipe in recipes)
        + append
        + ")"
    )
    ...
    **kwargs: Dict[Any, Any]
    ...
    with sp.Popen(
        ...
        **kwargs
        ...
        if returncode:
            logger.error(
                "COMMAND FAILED (exited with %s): %s", returncode, " ".join(masked_cmds)
            )
            if not live:
...
def newly_unblacklisted(config_file, recipe_folder, git_range):
    ...
    current = get_blacklist(
        yaml.safe_load(file_from_commit(git_range[1], config_file)), recipe_folder
    )
...
def get_package_paths(recipe, check_channels, force=False):
    ...
    for meta in existing_metas:
        logger.info(
            ...
            "it is in channel(s) and it is not forced.",
            meta.pkg_fn(),
        )
    ...
    def _load_channel_dataframe(self):
        ...
        def to_dataframe(json_data, meta_data):
            ...
            df = pd.DataFrame.from_dict(
                repo["packages"], "index", columns=self._load_columns
            )
