...
async def async_exec(
    command: str, *arguments: str, env: dict[str, str] | None = None
) -> None:
...
async def send_comment(session: ClientSession, issue_number: int, message: str) -> None:
    ...
    url = f"https://api.github.com/repos/bioconda/bioconda-recipes/issues/{issue_number}/comments"
    headers = {
        ...
        for job in res_wf_object["items"]:
            if job["name"].startswith("build_and_test-"):
                ...
                for artifact in res_object:
                    ...
                    if zipUrl.endswith(
                        (".conda", ".tar.bz2")
                    ):  # (currently excluding container images) or zipUrl.endswith(".tar.gz"):
                        artifacts.append((zipUrl, pkg))
        return artifacts
...
async def fetch_pr_sha_artifacts(
    session: ClientSession, pr: int, sha: str
    ...
    for check_run in check_runs["check_runs"]:
        if (
            "azure" not in artifact_sources
            and check_run["app"]["slug"] == "azure-pipelines"
            and check_run["name"].startswith("bioconda.bioconda-recipes (test_")
        ):
            # azure builds
            ...
            artifact_sources["azure"] = (
                zipFiles  # We've already fetched all possible artifacts from Azure
            )
        elif (
            "circleci" not in artifact_sources
            and check_run["app"]["slug"] == "circleci-checks"
        ):
            # Circle CI builds
            workflowId = safe_load(check_run["external_id"])["workflow-id"]
            zipFiles = await fetch_circleci_artifacts(session, workflowId)
            artifact_sources["circleci"] = (
                zipFiles  # We've already fetched all possible artifacts from CircleCI
            )
        elif (
            "github-actions" not in artifact_sources
            ...
            artifact_sources["github-actions"] = (
                zipFiles  # We've already fetched all possible artifacts from GitHub Actions
            )

    return artifact_sources


async def get_sha_for_status(job_context: dict[str, Any]) -> str | None:
    ...
    if not branches:
        return None
    sha: str | None = branches[0]["commit"]["sha"]
...
async def get_sha_for_check_suite_or_workflow(
    ...
    if event_source["conclusion"] != "success":
        return None
    sha: str | None = event_source.get("head_sha")
    if not sha:
...
async def get_sha_for_check_suite(job_context: dict[str, Any]) -> str | None:
    return await get_sha_for_check_suite_or_workflow(job_context, "check_suite")


async def get_sha_for_workflow_run(job_context: dict[str, Any]) -> str | None:
...
async def get_sha_for_status_check(job_context: dict[str, Any]) -> str | None:
    return await get_sha_for_status(job_context) or await get_sha_for_check_suite(
        job_context
    )
...
async def get_pr_comment(job_context: dict[str, Any]) -> tuple[int | None, str | None]:
    event = job_context["event"]
